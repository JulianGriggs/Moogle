...
struct
  ...
  *)
  let rank (g : G.graph) : NS.node_score_map = 
    	...
    	* raise an exception because we know that there should be an element. 
    	*)
  	let chooseRandom (ls: 'a list) : 'a = 
    	match ls with
    	| [] -> raise BAD_LOGIC
    	| _ -> List.nth ls (Random.int (List.length ls))
		in
        (* 
